...
from bert import tokenization_mecab as tokenization_ja
...
flags.DEFINE_string(
    "model_file", "./ja-checkpoint/wiki-ja.model", "The model file that the SentencePiece model was trained on."
)
    ...
    def _read_data(cls, input_file):
        ...
        with open(input_file, encoding='utf-8') as f:
            ...
            for line in f:
                ...
                if len(line) == 0 and words[-1] == '。':
                    # end of reading a sentence
...
class NerProcessor(DataProcessor):
    ...
    def get_pred_examples(self, data_dir):
        return self._create_example(
            self._read_data(os.path.join(data_dir, "prediction.txt")), "pred")

    def get_labels(self):
        return ['I-ORG', 'B-DAT', 'B-ORG', 'I-ART', 'I-MNY', 'B-TIM', 'I-PNT', 'I-PSN', 'I-DAT', 'O', 'B-PNT', 'B-PSN',
                'I-LOC', 'B-MNY', 'B-LOC', 'B-ART', 'I-TIM', "[CLS]", "[SEP]", "X"]
    ...
    def model_fn(features, labels, mode, params):
        ...
        if mode == tf.estimator.ModeKeys.TRAIN:
            ...
            def metric_fn(per_example_loss, label_ids, logits):
                ...
                pos_indices_list = list(range(num_labels))[1:18]  # do not contain [PAD] [X] [CLS]and [SEP]
...
def main(_):
    ...
    # tokenizer = tokenization_ja.FullTokenizer(
    #     model_file=FLAGS.model_file, vocab_file=FLAGS.vocab_file,
    #     do_lower_case=FLAGS.do_lower_case)
    path = ""

    tokenizer = tokenization_ja.MecabBertTokenizer("./checkpoint/vocab.txt", mecab_dict_path="/usr/lib/x86_64-linux-gnu/mecab/dic/mecab-ipadic-neologd")
    ...
    model_fn = model_fn_builder(
        ...
        num_labels=len(label_list) + 1,  # 1 for the [PAD]
    ...
    if FLAGS.do_eval:
        ...
        if FLAGS.do_predict:
            # token_path = os.path.join(FLAGS.output_dir, "token_test.txt")
            token_path = os.path.join(FLAGS.output_dir, "token_pred.txt")
            with open('./output/label2id.pkl', 'rb') as rf:
                label2id = pickle.load(rf)
                id2label = {value: key for key, value in label2id.items()}
            if os.path.exists(token_path):
            ...
            def serving_input_fn():
                ...
                label_ids = tf.placeholder(
                    tf.int32, [None, FLAGS.max_seq_length], name='label_ids')
            ...
            output_predict_file = os.path.join(FLAGS.output_dir, "label_pred.txt")
            with open(output_predict_file, 'w') as writer:
                for prediction in result:
                    output_line = "\n".join(id2label[id] for id in prediction if id != 0) + "\n"
                    writer.write(output_line)
